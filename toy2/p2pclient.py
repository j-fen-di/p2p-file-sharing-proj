...
class p2pclient:
            ...
            for action in self.actions:
                if (self.time == action["time"]):
                    ...
                    if action["code"] == "Q":
                        ...
                        self.query_client_for_content_list(action["client_id"])
                    elif action["code"] == "O":
                        self.query_client_for_known_client(action["client_id"])
                    elif action["code"] == "P":
    ...
    def query_client_for_known_client(self, client_id):
        ...
        if content_orig_list != None:
            ...
            log_dict = {"time": self.time, "text": "Client " + client_id + ": "}
            self.log.append(log_dict)
        return client_list
    ...
    def request_content(self, content_id):
        ...
        #self.content_originator_list is a dictionary content_id:(client_id, ip, port)
            ...
            content_list = self.query_client_for_content_list(client_id)
