...
class p2pbootstrapper:
    ...
    def client_thread(self, clientSocket, addr):
        ...
        if request[0] == "R":
            ...
            self.register_client(request[1], ip, port)
            clientSocket.close()
        elif request[0] == "U":
            self.deregister_client(request[1])
            clientSocket.close()
        elif request[0] == "L":
    ...
    def return_clients(self):
        ...
        ##############################################################################
        #return self.clients
        #regClients = self.clients.copy()
        # clientDict = {}
        # for client in regClients:
        #     if regClients[client][2] == "registered":
        #         clientDict[client] = regClients[client]
        # return clientDict
        return self.clients
    ...
    def start(self):
        ...
        if self.time == 0:
            ...
            for client in orderedDict:
                if orderedDict[client][2] == "registered":
